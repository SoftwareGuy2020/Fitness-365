...
public class FoodSearch implements SceneNavigation, Initializable {
	...
	private void filter() {
		...
		if (proteinRB.isSelected()) {
			mealsList = mController
					.filter(b -> ((comboBox == null || comboBox.equals(" ") || b.getGroup().equalsIgnoreCase(comboBox))
	...
	private void addItemtoFD() {
		...
		if (!servingSizeTF.getText().isEmpty()) {
			...
			FoodDiaryEntry entry = new FoodDiaryEntry(meal, servingSize, category, LocalDate.now(),
